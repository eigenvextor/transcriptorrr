...
def get_wav_path(m_id):
    ...
    if not Path(video_path).exists():
        print(f"video file {m_id} doesnt exist")
        return
        
    if Path(audio_path).exists():
        print(f"audio file {m_id} already exists")
        return audio_path
    else:
        command = f"ffmpeg -i {video_path} {audio_path}"
        try:
            _ = subprocess.run(command, shell=True, capture_output=True)
            print(f"audio file {m_id} created")
            return audio_path
        except:
            print(f"couldn't convert {m_id} video to .wav format")
            return
